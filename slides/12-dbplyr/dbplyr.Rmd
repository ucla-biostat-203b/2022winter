...
Here, we will import only one csv file `icustays.csv.gz` for demonstration purpose. Motivated students can write a bash script for loading all MIMIC-IV datafiles into a SQLite database and contribute to <https://github.com/MIT-LCP/mimic-iv>.
...
SQLite (0.08 seconds) was much faster than tibble (0.85 seconds). But SQLite is disk-based, while the tibble is in memory. Why is the discrepancy?
...
```
Apparently current version of ggplot2 (v3.3.5) works for `tbl_dbi/tbl_sql/tbl_lazy` now. 
